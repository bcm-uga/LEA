                       ...
                       bituint * dat)
{
        ...
        while (fgets(szbuff, max_char_per_line, m_File) && (j < M)) {
                // fill current line
                fill_line_geno_bituint(dat, Mp, N, j, nc, file_data, szbuff,
                                       m_File, I, nb);
                j++;
        }
...
void fill_line_geno_bituint(bituint * dat, int Mp, int N,
                            int j, int nc, char *file_data, char *szbuff,
                            FILE * m_File, int *I, double *nb)
{
        ...
        while (token != EOF && token != '\n' && i < N) {
                ...
                if (value == 9) {
                        if (I) {
                                ...
                                Rprintf
                                    ("Internal Error: your data file contains missing"
                                     " data.\n");
                                error(NULL);
                        }
                } else {
                        ...
                        if (c < nc) {
                                ...
                                Rprintf
                                    ("Error: Unknown element '%d' in the data file: %s.\n",
                                     value, file_data);
                                error(NULL);
                        }
...
void write_geno_bituint(char *file_data, int N, int nc, int Mp, int M,
                        bituint * dat)
{
        int i, j, jd, jm, jc, c;
        FILE *file = NULL;
