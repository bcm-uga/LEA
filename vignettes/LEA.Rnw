...
Genomic offset statistics use environmental and genomic data to predict shifts in adaptive traits without direct observations of those traits. By relying on GEA models, they are based on differences in allelic frequencies for two sets of conditions in an ecological niche. 

The {\tt R} package {\tt LEA} implements the genetic gap approach, which estimates are based on the covariance matrix of effect sizes obtained from an LFMM \citep{Gain_2022}.
...
Below, we compute the genetic gap at each sample locations, using all genomic loci in the genotype matrix. Note that the {\tt candidate.loci = TRUE} option allows us to use any subset of loci, in particular those  being above the significance level in the GEA analysis.  
