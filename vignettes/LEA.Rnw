...
{\tt LEA} is an {\tt R} package dedicated to landscape genomics and
    ecological association tests \citep{Frichot_2015}. {\tt LEA} can run analyses of
        ...
        environmental gradients (lfmm, lfmm2). {\tt LEA} is mainly based on optimized C programs
...
We fit an LFMM by using $K = 3$ latent factors. This value corresponds to the true value in the model, but is also easily recovered by a PCA screeplot. 
