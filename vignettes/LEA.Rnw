...
{\tt LEA} is an {\tt R} package dedicated to landscape genomics and
    ecological association tests \citep{Frichot_2015}. {\tt LEA} can run analyses of
        ...
        environmental gradients (lfmm). {\tt LEA} can also evaluate association
        with phenotypic traits. The program is mainly based on optimized C programs
        that can scale with the dimension of large data sets.  


\section{Introduction} 
The goal of this tutorial is to give an overview of the main functionalities of the {\tt R} package {\tt LEA}. It shows the main steps of analysis, including 1) analyzing population structure and preparing a genotypic matrix for genomewide association studies, 2) fitting latent factor mixed models to the data and extracting candidate regions of interest.    
...
are around $K = 5$ major components in the data ($\approx 6$ genetic clusters). 
...
plot(tw$percentage)
@
\caption{Screeplot for the percentage of variance explained by each component in a PCA of the genetic data. The knee at $K = 5$ indicates that there are around 6 major genetic clusters in the data.}
\end{figure}
...
The {\tt snmf} function estimates an entropy criterion that evaluates the quality of fit of the statistical model to the data using a cross-validation technique (Figure 2). The entropy criterion can help choosing the number of ancestral populations that best explains the genotypic data \citep{Alexander_2011, Frichot_2014}. Here we have a minimum at $K = 3$ or $K = 4$, suggesting 3-4 genetic clusters. If the plot shows a less clear pattern, choosing the "knee" value is a generally good approach. The number of ancestral populations is closely linked to the number of principal components. Both numbers can help determining the number of latent factors when correcting for confounding effects due to population structure in ecological association tests. 
...
\newline
\newline
Testing all $K$ values in a large range, from 1 to 20 for example, is generally useless. A careful analysis of population structure and estimates of the number of ancestral populations contributing to the genetic data indicates the range of values to be explored. For example the {\tt snmf} program estimates 4 ancestral populations, then running {\tt lfmm} for $K = 3-7$ often provides good results.
